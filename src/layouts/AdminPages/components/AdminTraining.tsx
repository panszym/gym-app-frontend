...
import { DisplayTraining } from "./AdminDisplayTraining";
...
export const Training = () => {
  ...
  useEffect(() => {
    const fetchTraining = async () => {
      ...
      if (searchUrl === "") {
        ...
        let searchPage = searchUrl.replace(
          "<pageNumber>",
          `${currentPage - 1}`
        );
  ...
  const categoryField = (value: string) => {
    ...
    if (
      ...
      setSearchUrl(
        `/category?category=${value}&page=<pageNumber>&size=${trainingPerPage}`
      );
    } else {
  ...
  const statusField = (value: string) => {
    ...
    if (value === "ACTIVE" || value === "INACTIVE" || value === "FULL") {
      setStatusSelection(value);
      setSearchUrl(
        `/status?status=${value}&page=<pageNumber>&size=${trainingPerPage}`
      );
    } else {
      setStatusSelection("ALL");
      setSearchUrl(`?page=<pageNumber>&size=${trainingPerPage}`);
