...
export const removeParticipantFromTRaining = (
  trainingCode: string,
  clientId: string
) => {
  return apiTraining.post(
    `training/remove/${trainingCode}/clients/${clientId}`
  );
};

export const getClientTraining = (email: string) => {
  return apiTraining.get<TrainingModel[]>(`training/client`, {
    params: { email: email },
  });
};
